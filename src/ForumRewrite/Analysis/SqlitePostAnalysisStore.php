...
final class SqlitePostAnalysisStore implements PostAnalysisStore
{
    ...
    public function find(string $postId, string $contentHash): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT post_id, content_hash, status, requested_at, completed_at, provider, provider_model, post_summary,
                    provider_request_id, moderation_json, engagement_json, quality_json, respondability_json, related_content_json, related_content_assessment_json, raw_response_json,
    ...
    public function saveComplete(string $postId, string $contentHash, array $analysis): array
    {
        ...
        $row = [
            ...
            'related_content_assessment_json' => $this->encode($analysis['related_content_assessment'] ?? []),
    ...
    private function ensureSchema(): void
    {
        $this->pdo->exec(
            'CREATE TABLE IF NOT EXISTS post_analyses (
                ...
                related_content_assessment_json TEXT NOT NULL DEFAULT \'{}\',
        ...
        if (!$this->columnExists('post_analyses', 'related_content_assessment_json')) {
            $this->pdo->exec('ALTER TABLE post_analyses ADD COLUMN related_content_assessment_json TEXT NOT NULL DEFAULT \'{}\'');
        }
    ...
    private function upsert(array $row): void
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO post_analyses (
                ...
                provider_request_id, post_summary, moderation_json, engagement_json, quality_json, respondability_json, related_content_json, related_content_assessment_json, raw_response_json,
                failure_code, failure_message, retry_after
             ) VALUES (
                :post_id, :content_hash, :status, :requested_at, :completed_at, :provider, :provider_model,
                :provider_request_id, :post_summary, :moderation_json, :engagement_json, :quality_json, :respondability_json, :related_content_json, :related_content_assessment_json, :raw_response_json,
             ...
             ON CONFLICT(post_id, content_hash) DO UPDATE SET
                ...
                related_content_assessment_json = excluded.related_content_assessment_json,
    ...
    private function hydrate(array $row): array
    {
        return [
            ...
            'related_content_assessment' => $this->decode((string) ($row['related_content_assessment_json'] ?? '{}')),
