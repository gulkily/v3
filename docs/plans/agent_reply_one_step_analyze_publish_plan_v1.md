...
- 2026-05-12: Step 2 complete - wired `POST /api/analyze_post` to publish through the shared helper when config, completed analysis, and gates allow it; added compact allowlisted `agent_reply_*` response mapping with local skipped-case summaries. Verified with `php -l src/ForumRewrite/Application.php`.
- 2026-05-12: Step 3 complete - updated `public/assets/post_analysis.js` so `work === "analyze"` calls only `/api/analyze_post`, adapts embedded `agent_reply_*` fields with `agentReplyResultFromAnalysis()`, and leaves `work === "publish"` on `/api/generate_agent_reply`. Verified with `node --check public/assets/post_analysis.js`.
- 2026-05-12: Step 4 complete - tightened `agentReplyWorkForPost()` generated-response row handling so only an unposted `complete` row can render `publish`; posted, pending, posting, failed, and unknown row states render no automatic work. Verified with `php -l src/ForumRewrite/Application.php`.
- 2026-05-12: Step 5 complete - updated smoke coverage for one-step `/api/analyze_post` posting, embedded compact response fields, canonical reply creation, generated-response metadata, idempotent repeat analysis, disabled config, gate-failure privacy, and JS source branching. Verified with `php tests/run.php`.
- 2026-05-12: Step 6 complete - final verification passed with `php tests/run.php` and `node --check public/assets/post_analysis.js`.
