...
- Selecting an individual action (in the other 5 views) shows only files in its own provenance chain: its own record(s), its detached signature (if any), and the signer's public key (if any) - not the rest of the commit. Most actions have exactly one record; identity bootstraps are a compound action that also establishes a separate identity record, so their relevant set is record + identity record + signature + key.
...
## Success Criteria
- An action's detail view only ever shows files in that action's own provenance chain (record(s), signature, public key) - regardless of how many other, unrelated files the underlying commit touched.
