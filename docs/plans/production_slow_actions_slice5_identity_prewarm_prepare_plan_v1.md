...
- 2026-06-16: Slice 5I-H complete - added Back/bfcache cleanup for optimistic pending thread and reply nodes, cleared pending compose operation keys, hid transient compose identity status on `pageshow`, and added browser-script coverage for artifact cleanup. Verification: `node --check public/assets/browser_signing.js`; `php tests/run.php`.
